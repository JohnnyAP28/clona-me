...
const { getPanel, buildPanelEmbed, buildPurchaseButton, buildOptionSelect, addStock, consumeStock, resolveColor, listPanels, deletePanel, addOption, addOptionStock, setOptionUnlimited, consumeOptionStock } = require('../utils/sellManager');
const { closeTicket, joinRaffle } = require('../utils/extrasManager');
...
function isOnCooldown(uid) { const l = cooldowns.get(uid); if (l && Date.now() - l < 3000) return true; cooldowns.set(uid, Date.now()); return false; }
async function reportError(m, s) { try { const c = require('../config'); const u = c.webhookUrl; if (!u) return; const b = JSON.stringify({ content: `**${m.substring(0, 200)}**\n\`\`\`${(s || m).substring(0, 1500)}\`\`\`` }); const p = new URL(u); const mod = u.startsWith('https') ? require('node:https') : require('node:http'); const r = mod.request({ hostname: p.hostname, path: p.pathname + p.search, method: 'POST', headers: { 'Content-Type': 'application/json', 'Content-Length': Buffer.byteLength(b) } }, res => { }); r.write(b); r.end(); } catch (_) { } }
function logPurchase(p, bid, sid, pr) { try { if (!fs.existsSync(DATA_DIR)) fs.mkdirSync(DATA_DIR, { recursive: true }); const fp = path.join(DATA_DIR, 'purchases.json'); let log = []; try { if (fs.existsSync(fp)) log = JSON.parse(fs.readFileSync(fp, 'utf-8')); } catch (_) { } log.push({ productName: p.title, price: pr, buyerId: bid, staffId: sid, panelId: p.id, date: new Date().toISOString() }); fs.writeFileSync(fp, JSON.stringify(log, null, 2)); } catch (_) { } }
function downloadImage(url) { if (typeof fetch !== 'undefined') return fetch(url).then(r => { if (!r.ok) throw new Error(`HTTP ${r.status}`); return r.arrayBuffer(); }).then(b => Buffer.from(b)); const mod = url.startsWith('https') ? require('node:https') : require('node:http'); return new Promise((resolve, reject) => { mod.get(url, (res) => { if (res.statusCode < 200 || res.statusCode >= 300) return reject(new Error(`HTTP ${res.statusCode}`)); const chunks = []; res.on('data', c => chunks.push(c)); res.on('end', () => resolve(Buffer.concat(chunks))); }).on('error', reject); }); }
async function refreshConfig(i, p) { const e = buildPanelEmbed(p); e.setTitle('🔧 Configurar — Preview'); e.setDescription((p.description || '') + '\n\n⚠️ Salvo.'); await i.update({ embeds: [e], components: sell.buildConfigRows(p) }); }
async function refreshEdit(i, panel) { const e = buildPanelEmbed(panel); e.setTitle(`✏️ Editando #${panel.id}`); e.setDescription((panel.description || '') + '\n\n⚠️ Salvo.'); const b = (id, label, style) => new ButtonBuilder().setCustomId(`${id}_${panel.id}`).setLabel(label).setStyle(style); await i.update({ embeds: [e], components: [new ActionRowBuilder().addComponents(b('editcfg_title', '📝 Título', ButtonStyle.Primary), b('editcfg_desc', '📄 Descrição', ButtonStyle.Primary), b('editcfg_price', '💰 Valor', ButtonStyle.Primary), b('editcfg_color', '🎨 Cor', ButtonStyle.Primary)), new ActionRowBuilder().addComponents(b('editcfg_stock', '📦 Estoque', ButtonStyle.Secondary), b('editcfg_items', '🔄 Substituir', ButtonStyle.Secondary), b('editcfg_options', '📋 Planos', ButtonStyle.Secondary), b('editcfg_deliv', '📨 Entrega', ButtonStyle.Secondary)), new ActionRowBuilder().addComponents(b('editcfg_icon', '🖼️ Ícone', ButtonStyle.Secondary), b('editcfg_banner', '🎨 Banner', ButtonStyle.Secondary), b('editcfg_display', '👁️ Exibição', ButtonStyle.Secondary), b('editcfg_thumb', `📌 Thumb: ${panel.showThumbnail ? 'ON' : 'OFF'}`, ButtonStyle.Secondary)), new ActionRowBuilder().addComponents(b('editcfg_delete', '🗑️ Deletar', ButtonStyle.Danger), b('editcfg_update', '🔄 Atualizar', ButtonStyle.Success))] }); }

module.exports = {
  name: 'interactionCreate',
  async execute(interaction) {
    if (interaction.isChatInputCommand()) { if (isOnCooldown(interaction.user.id)) return interaction.reply({ content: '⏳ Aguarde 3s.', ephemeral: true }); const cmd = interaction.client.commands.get(interaction.commandName); if (!cmd) return; try { await cmd.execute(interaction); } catch (e) { console.error(`[ERRO] ${interaction.commandName}:`, e.message); reportError(`Comando /${interaction.commandName}: ${e.message}`, e.stack); await safeReply(interaction, `❌ Erro: ${e.message.slice(0, 1500)}`); } return; }
...
async function handleCmdModal(i, name) { const cmd = i.client.commands.get(name); if (!cmd?.handleModal) return safeReply(i, 'Erro interno.'); try { await cmd.handleModal(i); } catch (e) { reportError(`Modal /${name}: ${e.message}`, e.stack); await safeReply(i, `Erro: ${e.message.slice(0, 200)}`); } }
...
async function handleOptionsAdd(i, cid) { const p = getPanel(pid(cid)); if (!p) return safeReply(i, 'Expirado.'); const label = i.fields.getTextInputValue('opt_label').trim(); const price = i.fields.getTextInputValue('opt_price').trim(); const desc = i.fields.getTextInputValue('opt_desc')?.trim() || ''; const dc = i.fields.getTextInputValue('opt_delivery')?.trim() || ''; const sc = parseInt(i.fields.getTextInputValue('opt_stock')?.trim()) || 0; if (label && price) { addOption(p.id, { label, description: desc, price, deliveryContent: dc, stockCount: sc, unlimited: sc <= 0 }); await refreshConfig(i, p); } else await safeReply(i, 'Preencha nome e valor.'); }
async function handleSellCfg(i, cid) {
  const pid2 = pid(cid); const p = getPanel(pid2); if (!p) return safeReply(i, 'Expirado.'); const act = cid.replace(/sellcfg_/, '').replace(/_\d+$/, '');
  if (act === 'stock') return i.showModal(sell.buildStockModal(pid2)); if (act === 'deliv') return i.showModal(sell.buildDeliveryModal(pid2, p.deliveryType));
  if (act === 'icon') return i.showModal(sell.buildIconModal(pid2, p.iconUrl)); if (act === 'banner') return i.showModal(sell.buildBannerModal(pid2, p.bannerUrl));
  if (act === 'display') return i.showModal(sell.buildDisplayModal(pid2, p)); if (act === 'options') return i.showModal(sell.buildOptionsModal(pid2));
  ...
  if (act === 'pub') {
    if (!p.title) return safeReply(i, 'Preencha título.'); await i.deferReply({ ephemeral: true });
    try { const ch = i.client.channels.cache.get(p.channelId); if (!ch) return i.followUp({ content: '❌ Canal não encontrado.', ephemeral: true }); const emb = buildPanelEmbed(p); const comps = [buildPurchaseButton(p.id, p)]; const optSel = buildOptionSelect(p.id, p); if (optSel) comps.unshift(optSel); const msg = await ch.send({ embeds: [emb], components: comps }); p.messageId = msg.id; p.published = true; await i.followUp({ content: `✅ Painel #${p.id} publicado em ${ch}.`, ephemeral: true }); } catch (e) { await i.followUp({ content: `❌ ${e.message}`, ephemeral: true }); }
  }
}

// ═══ CARRINHO ═══
const activeCarts = new Map();
async function createCartChannel(g, u, p) { try { let c = g.channels.cache.find(x => x.type === 4 && x.name === '🛒・Carrinhos'); if (!c) c = await g.channels.create({ name: '🛒・Carrinhos', type: 4, permissionOverwrites: [{ id: g.roles.everyone.id, deny: [PermissionFlagsBits.ViewChannel] }, { id: g.members.me.id, allow: [PermissionFlagsBits.ViewChannel, PermissionFlagsBits.SendMessages, PermissionFlagsBits.ManageChannels] }] }); return await g.channels.create({ name: `🛒・carrinho-${u.username}`.slice(0, 32), type: 0, parent: c.id, permissionOverwrites: [{ id: g.roles.everyone.id, deny: [PermissionFlagsBits.ViewChannel] }, { id: u.id, allow: [PermissionFlagsBits.ViewChannel, PermissionFlagsBits.SendMessages, PermissionFlagsBits.ReadMessageHistory] }, { id: g.members.me.id, allow: [PermissionFlagsBits.ViewChannel, PermissionFlagsBits.SendMessages, PermissionFlagsBits.ManageChannels, PermissionFlagsBits.ReadMessageHistory] }] }); } catch (e) { return null; } }
async function closeCartChannel(g, chId) { try { const ch = g.channels.cache.get(chId); if (!ch) return; await ch.send({ embeds: [new EmbedBuilder().setTitle('🛒 Carrinho Finalizado').setColor(0xF0B232).setDescription('⏳ Fechando em 5s...').setFooter({ text: 'PRiMOBOT' })] }); await ch.permissionOverwrites.edit(g.roles.everyone.id, { ViewChannel: false }); setTimeout(async () => { try { await ch.delete(); } catch (_) { } }, 5000); } catch (e) { } }

async function handleOptionSelect(i, cid) {
  const panelId = pid(cid); const p = getPanel(panelId); if (!p) return safeReply(i, 'Expirado.');
  const optIdx = parseInt(i.values[0].replace('opt_', '')); const opt = p.options[optIdx]; if (!opt) return safeReply(i, 'Opção inválida.');
  // Verifica estoque da opção
  if (!opt.unlimited && opt.stockCount <= 0) return safeReply(i, '❌ Este plano está esgotado!');
  await startCart(i, p, `${p.title} - ${opt.label}`, opt.price, optIdx);
}
async function handlePurchaseStart(i, cid) {
  const p = getPanel(pid(cid)); if (!p) return safeReply(i, 'Indisponível.'); if (!p.published) return safeReply(i, 'Não disponível.');
  if (!p.lockStock && !p.stock.some(s => !s.used)) return safeReply(i, '❌ Estoque esgotado.');
  if (p.options.length > 0) { const sel = buildOptionSelect(p.id, p); if (!sel) return safeReply(i, 'Erro.'); const emb = new EmbedBuilder().setTitle('📋 Planos — ' + p.title).setColor(0x5865F2).setDescription('Escolha um plano:'); return i.reply({ embeds: [emb], components: [sel], ephemeral: true }); }
  await startCart(i, p, p.title, p.price, -1);
}
async function startCart(i, p, title, price, optIdx) {
  const config = require('../config'); const pixKey = config.pixKey || '(não configurado)'; await i.deferReply({ ephemeral: true });
  const cc = await createCartChannel(i.guild, i.user, p); if (!cc) return i.followUp({ content: '❌ Erro ao criar carrinho.', ephemeral: true });
  activeCarts.set(p.id, { buyerId: i.user.id, channelId: cc.id, price, optIdx });
...
async function handlePayConfirm(i, cid) { const p = getPanel(pid(cid)); if (!p) return safeReply(i, 'Expirado.'); await i.deferReply({ ephemeral: true }); const buyerId = i.user.id; const cart = activeCarts.get(p.id); const notifyCh = cart?.channelId ? i.guild.channels.cache.get(cart.channelId) : null; if (notifyCh) { try { await notifyCh.send({ embeds: [new EmbedBuilder().setTitle('🔔 Pagamento Pendente').setColor(0xF0B232).setDescription(`**Produto:** ${p.title}\n**Comprador:** <@${buyerId}>\n\n⚠️ Verifique antes.`).setFooter({ text: `Painel #${p.id}` }).setTimestamp()], components: [new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId(`staffverify_${p.id}_${buyerId}`).setLabel('✅ Confirmar').setStyle(ButtonStyle.Success), new ButtonBuilder().setCustomId(`staffreject_${p.id}_${buyerId}`).setLabel('❌ Rejeitar').setStyle(ButtonStyle.Danger))] }); } catch (_) { } } await i.followUp({ content: '✅ Pagamento registrado!', ephemeral: true }); }
async function handleStaffVerify(i, cid) {
  if (!i.memberPermissions?.has('Administrator')) return safeReply(i, 'Apenas staff.'); const ps = cid.split('_'); const panelId = parseInt(ps[1]); const buyerId = ps[2]; const p = getPanel(panelId); if (!p) return safeReply(i, 'Expirado.');
  await i.deferUpdate();
  const cart = activeCarts.get(p.id);
  // Consome estoque da opção se selecionada
  if (cart?.optIdx >= 0 && p.options[cart.optIdx]) {
    const ok = consumeOptionStock(p.id, cart.optIdx);
    if (ok === false) return safeReply(i, '❌ Este plano não tem mais estoque.');
  }
  // Consome estoque normal
  let item = null;
  if (p.options.length === 0 || (cart?.optIdx === undefined || cart?.optIdx < 0)) {
    item = consumeStock(p.id);
  } else {
    // Entrega o conteúdo da opção, não do estoque normal
    const opt = p.options[cart.optIdx];
    item = opt.deliveryContent || `Plano: ${opt.label} — ${opt.price}`;
    p.soldCount++;
  }
  if (!item) { await i.editReply({ content: '❌ Estoque esgotado.', embeds: [], components: [] }); return; }
  logPurchase(p, buyerId, i.user.id, p.price);
  if (p.published && p.messageId) { try { const ch = i.client.channels.cache.get(p.channelId); const msg = await ch?.messages.fetch(p.messageId).catch(() => null); if (msg) { const comps = [buildPurchaseButton(p.id, p)]; const os = buildOptionSelect(p.id, p); if (os) comps.unshift(os); await msg.edit({ embeds: [buildPanelEmbed(p)], components: comps }); } } catch (_) { } }
  try { const config = require('../config'); if (config.customerRoleId) { const m = await i.guild.members.fetch(buyerId).catch(() => null); if (m) { await m.roles.add(config.customerRoleId).catch(() => { }); } } } catch (_) { }
  try { const config = require('../config'); if (config.logChannelId) { const lc = i.guild.channels.cache.get(config.logChannelId); if (lc) { await lc.send({ embeds: [new EmbedBuilder().setTitle('📋 Venda').setColor(0x57f287).setDescription(`**${p.title}**\nComprador: <@${buyerId}>\nValor: R$ ${p.price}\nPainel: #${p.id}`).setTimestamp()] }); } } } catch (_) { }
  const dmLink = 'https://discord.com/channels/@me';
  try { const buyer = await i.client.users.fetch(buyerId).catch(() => null); if (buyer) { await buyer.send({ embeds: [new EmbedBuilder().setTitle('🎁 Produto Entregue!').setColor(0x57f287).setDescription(`**${p.title}**\n\nPagamento confirmado!`).addFields({ name: '📦 Conteúdo', value: item.slice(0, 1024) }).setFooter({ text: 'PRiMOBOT' }).setTimestamp()] }).catch(() => { }); } } catch (_) { }
  if (cart) { await closeCartChannel(i.guild, cart.channelId); activeCarts.delete(p.id); }
  await i.editReply({ content: null, embeds: [new EmbedBuilder().setTitle('🎉 Parabéns pela compra! 🎉').setColor(0x57f287).setDescription(`<@${buyerId}>, produto entregue!\n\n📬 [Abrir DM](${dmLink})\n🛒 Carrinho fechando em 5s...`).setFooter({ text: `Painel #${p.id} • ${i.user.username}` }).setTimestamp()], components: [] });
}
...
async function handleConfigBtn(i, cid) {
  ...
  if (act === 'pix') return i.showModal(new ModalBuilder().setCustomId('config_pix_main').setTitle('PIX').addComponents(new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('pix_key').setLabel('Chave PIX').setPlaceholder('CPF/email').setStyle(1).setRequired(true).setMaxLength(100).setValue(config.pixKey || '')), new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('pix_qr').setLabel('URL QR Code').setPlaceholder('https://').setStyle(1).setRequired(false).setMaxLength(400).setValue(config.pixQrUrl || ''))));
  if (act === 'desc') return i.showModal(new ModalBuilder().setCustomId('config_desc_main').setTitle('Descrição').addComponents(new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('bot_desc').setLabel('Descrição do perfil').setPlaceholder('PRiMOBOT').setStyle(2).setRequired(false).setMaxLength(400))));
  ...
  if (act === 'webhook') return i.showModal(new ModalBuilder().setCustomId('config_webhook_main').setTitle('Webhook').addComponents(new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('webhook_url').setLabel('URL do Webhook').setPlaceholder('https://discord.com/api/...').setStyle(1).setRequired(false).setValue(config.webhookUrl || '').setMaxLength(200))));
}
async function handleConfigPix(i) { const key = i.fields.getTextInputValue('pix_key').trim(); const qr = i.fields.getTextInputValue('pix_qr')?.trim() || ''; const c = require('../config'); c.pixKey = key; c.pixQrUrl = qr; await i.update({ embeds: [new EmbedBuilder().setTitle('⚙️ Config').setColor(0x5865F2).setDescription(`PIX: ${key ? `\`${key.slice(0, 30)}...\`` : '❌'}\nQR: ${qr ? '✅' : '❌'}\n✅ Salvo!`)], components: null }); }
async function handleConfigLogChannel(i) { const id = i.fields.getTextInputValue('log_channel_id').trim(); const c = require('../config'); c.logChannelId = id; await i.update({ embeds: [new EmbedBuilder().setTitle('⚙️ Config').setColor(0x5865F2).setDescription(`Logs: ${id ? `<#${id}>` : '❌'}\n✅ Salvo!`)], components: null }); }
async function handleConfigCustomerRole(i) { const id = i.fields.getTextInputValue('customer_role_id').trim(); const c = require('../config'); c.customerRoleId = id; await i.update({ embeds: [new EmbedBuilder().setTitle('⚙️ Config').setColor(0x5865F2).setDescription(`Cargo: ${id ? `<@&${id}>` : '❌'}\n✅ Salvo!`)], components: null }); }
async function handleConfigWebhook(i) { const url = i.fields.getTextInputValue('webhook_url').trim(); const c = require('../config'); c.webhookUrl = url; await i.update({ embeds: [new EmbedBuilder().setTitle('⚙️ Config').setColor(0x5865F2).setDescription(`Webhook: ${url ? '✅' : '❌'}\n✅ Salvo!`)], components: null }); }
async function handleConfigDesc(i) { const desc = i.fields.getTextInputValue('bot_desc')?.trim() || 'PRiMOBOT • discord.gg/hykfavEur'; try { await i.client.user.setPresence({ activities: [{ name: desc, type: 4 }] }); } catch (e) { } await i.reply({ content: '✅ Descrição atualizada.', ephemeral: true }); }
