...
const { getPanel, buildPanelEmbed, buildPurchaseButton, buildOptionSelect, addStock, consumeStock, resolveColor, listPanels, deletePanel, addOption, removeOption } = require('../utils/sellManager');
...
async function refreshConfig(i, panel) {
  const embed = buildPanelEmbed(panel); embed.setTitle('🔧 Configurar Painel — Preview');
...
async function refreshEditMenu(i, panel) {
  const embed = buildPanelEmbed(panel); embed.setTitle(`✏️ Editando Painel #${panel.id}`);
  embed.setDescription((panel.description||'')+'\n\n⚠️ Alterações salvas automaticamente.');
  const r1 = new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId(`editcfg_title_${panel.id}`).setLabel('📝 Título').setStyle(ButtonStyle.Primary),new ButtonBuilder().setCustomId(`editcfg_desc_${panel.id}`).setLabel('📄 Descrição').setStyle(ButtonStyle.Primary),new ButtonBuilder().setCustomId(`editcfg_price_${panel.id}`).setLabel('💰 Valor').setStyle(ButtonStyle.Primary),new ButtonBuilder().setCustomId(`editcfg_color_${panel.id}`).setLabel('🎨 Cor').setStyle(ButtonStyle.Primary));
  const r2 = new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId(`editcfg_stock_${panel.id}`).setLabel('📦 Estoque').setStyle(ButtonStyle.Secondary),new ButtonBuilder().setCustomId(`editcfg_items_${panel.id}`).setLabel('🔄 Substituir').setStyle(ButtonStyle.Secondary),new ButtonBuilder().setCustomId(`editcfg_deliv_${panel.id}`).setLabel('📨 Entrega').setStyle(ButtonStyle.Secondary),new ButtonBuilder().setCustomId(`editcfg_icon_${panel.id}`).setLabel('🖼️ Ícone').setStyle(ButtonStyle.Secondary));
  const r3 = new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId(`editcfg_banner_${panel.id}`).setLabel('🎨 Banner').setStyle(ButtonStyle.Secondary),new ButtonBuilder().setCustomId(`editcfg_display_${panel.id}`).setLabel('👁️ Exibição').setStyle(ButtonStyle.Secondary),new ButtonBuilder().setCustomId(`editcfg_thumb_${panel.id}`).setLabel(`📌 Thumb: ${panel.showThumbnail?'ON':'OFF'}`).setStyle(ButtonStyle.Secondary),new ButtonBuilder().setCustomId(`editcfg_delete_${panel.id}`).setLabel('🗑️ Deletar').setStyle(ButtonStyle.Danger));
  const r4 = new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId(`editcfg_update_${panel.id}`).setLabel('🔄 Atualizar').setStyle(ButtonStyle.Success));
...
module.exports = {
  ...
  async execute(interaction) {
    if (interaction.isChatInputCommand()) { const cmd=interaction.client.commands.get(interaction.commandName); if(!cmd){console.error(`[ERRO] Cmd: ${interaction.commandName}`);return;} try{await cmd.execute(interaction);}catch(e){console.error(`[ERRO] ${interaction.commandName}:`,e.message);const r={content:`❌ Erro: ${e.message.slice(0,1500)}`,ephemeral:true};try{if(interaction.replied||interaction.deferred)await interaction.followUp(r);else await interaction.reply(r);}catch(_){}} return; }
    if (interaction.isModalSubmit()) { const cid=interaction.customId;
      ...
      if(cid.startsWith('sell_options_add_')) return handleOptionsAdd(interaction,cid);
      ...
      if(cid==='config_logchannel_main') return handleConfigLogChannel(interaction);
      if(cid==='config_customerrole_main') return handleConfigCustomerRole(interaction);
    }
...
async function handleOptionSelect(i,cid){
  ...
  try{await cartChannel.send({embeds:[emb],components:[new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId(`paycopy_${p.id}`).setLabel('📋 Copiar PIX').setStyle(ButtonStyle.Secondary),new ButtonBuilder().setCustomId(`payconfirm_${p.id}`).setLabel('✅ Já paguei').setStyle(ButtonStyle.Success),new ButtonBuilder().setCustomId(`paycancel_${p.id}`).setLabel('❌ Cancelar').setStyle(ButtonStyle.Danger))]});}catch(_){}
  await i.followUp({content:`🛒 **Carrinho criado!** Continue no canal: ${cartChannel}`,ephemeral:true});
}

async function handlePurchaseStart(i,cid){
  const p=getPanel(pid(cid));if(!p)return safeReply(i,'Indisponível.');if(!p.published)return safeReply(i,'Painel não disponível.');
  if(!p.lockStock&&!p.stock.some(s=>!s.used))return safeReply(i,'❌ Estoque esgotado.');
  // Se tem opções/planos, manda o select menu em vez de ir direto
  if(p.options.length>0){
    const sel=buildOptionSelect(p.id,p);if(!sel)return safeReply(i,'Erro ao carregar planos.');
    const config=require('../config');
    const emb=new EmbedBuilder().setTitle('📋 Escolha seu Plano — '+p.title).setColor(0x5865F2)
      .setDescription('Selecione o plano desejado abaixo:').setFooter({text:'Clona-Me'});
    return i.reply({embeds:[emb],components:[sel],ephemeral:true});
  }
  // Sem opções — fluxo normal com carrinho
  const config=require('../config');const pixKey=config.pixKey||'(não configurado)';
  await i.deferReply({ephemeral:true});
  const cartChannel=await createCartChannel(i.guild,i.user,p);if(!cartChannel)return i.followUp({content:'❌ Erro ao criar carrinho.',ephemeral:true});
...
async function handlePayConfirm(i,cid){const p=getPanel(pid(cid));if(!p)return safeReply(i,'Expirado.');await i.deferReply({ephemeral:true});const buyerId=i.user.id;const cart=activeCarts.get(p.id);const notifyCh=cart?.channelId?i.guild.channels.cache.get(cart.channelId):null;const optLabel=cart?.selectedOption?`\n**Plano:** ${cart.selectedOption.label} (${cart.selectedOption.price})`:'';if(notifyCh){try{await notifyCh.send({embeds:[new EmbedBuilder().setTitle('🔔 Pagamento Pendente').setColor(0xF0B232).setDescription(`**Produto:** ${p.title}${optLabel}\n**Comprador:** <@${buyerId}>\n\n⚠️ Verifique antes de confirmar.`).setFooter({text:`Painel #${p.id}`}).setTimestamp()],components:[new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId(`staffverify_${p.id}_${buyerId}`).setLabel('✅ Confirmar').setStyle(ButtonStyle.Success),new ButtonBuilder().setCustomId(`staffreject_${p.id}_${buyerId}`).setLabel('❌ Rejeitar').setStyle(ButtonStyle.Danger))]});}catch(_){}}await i.followUp({content:'✅ Pagamento registrado! A staff verificará em instantes.',ephemeral:true});}

async function handleStaffVerify(i,cid){
  if(!i.memberPermissions?.has('Administrator'))return safeReply(i,'Apenas staff.');
  const parts=cid.split('_');const panelId=parseInt(parts[1]);const buyerId=parts[2];const p=getPanel(panelId);if(!p)return safeReply(i,'Expirado.');
  await i.deferUpdate();const item=consumeStock(p.id);
  if(!item){await i.editReply({content:'❌ Estoque esgotado.',embeds:[],components:[]});return;}
  if(p.published&&p.messageId){try{const ch=i.client.channels.cache.get(p.channelId);const msg=await ch?.messages.fetch(p.messageId).catch(()=>null);if(msg){const comps=[buildPurchaseButton(p.id,p)];const optSel=buildOptionSelect(p.id,p);if(optSel)comps.unshift(optSel);await msg.edit({embeds:[buildPanelEmbed(p)],components:comps});}}catch(_){}}
  // Dar cargo de cliente automaticamente
  try{const config=require('../config');if(config.customerRoleId){const member=await i.guild.members.fetch(buyerId).catch(()=>null);if(member){await member.roles.add(config.customerRoleId).catch(()=>{});}}}catch(_){}
  // Logs de vendas
  try{const config=require('../config');if(config.logChannelId){const logCh=i.guild.channels.cache.get(config.logChannelId);if(logCh){await logCh.send({embeds:[new EmbedBuilder().setTitle('📋 Venda Registrada').setColor(0x57f287).setDescription(`**Produto:** ${p.title}\n**Comprador:** <@${buyerId}> (${buyerId})\n**Valor:** ${p.price||'N/D'}\n**Painel:** #${p.id}\n**Staff:** ${i.user}`).setTimestamp()]});}}}catch(_){}
  const dmLink='https://discord.com/channels/@me';
  try{const buyer=await i.client.users.fetch(buyerId).catch(()=>null);if(buyer){await buyer.send({embeds:[new EmbedBuilder().setTitle('🎁 Produto Entregue!').setColor(0x57f287).setDescription(`**${p.title}**\n\nSeu pagamento foi confirmado! Aqui está seu produto:`).addFields({name:'📦 Conteúdo',value:item.slice(0,1024)}).setFooter({text:'PRIMO Store • Obrigado por comprar!'}).setTimestamp()]}).catch(()=>{});}}catch(_){}
  const cart=activeCarts.get(p.id);if(cart){await closeCartChannel(i.guild,cart.channelId);activeCarts.delete(p.id);}
  await i.editReply({content:null,embeds:[new EmbedBuilder().setTitle('🎉 Parabéns pela compra! 🎉').setColor(0x57f287).setDescription(`<@${buyerId}>, seu produto foi entregue com sucesso!\n\n📬 **Acesse seu DM:** [Clique aqui](${dmLink})\n🛒 Carrinho fechando em 5s...`).setFooter({text:`Painel #${p.id} • Confirmado por ${i.user.username}`}).setTimestamp()],components:[]});
}
async function handleStaffReject(i,cid){if(!i.memberPermissions?.has('Administrator'))return safeReply(i,'Apenas staff.');const parts=cid.split('_');const panelId=parseInt(parts[1]);const buyerId=parts[2];const cart=activeCarts.get(panelId);if(cart){await closeCartChannel(i.guild,cart.channelId);activeCarts.delete(panelId);}await i.update({content:`❌ Pagamento do painel #${panelId} de <@${buyerId}> rejeitado.`,embeds:[],components:[]});}
...
async function handleEditTitle(i,cid){const p=getPanel(pid(cid));if(!p)return safeReply(i,'Expirado.');p.title=i.fields.getTextInputValue('new_title').trim();await refreshEditMenu(i,p);}
async function handleEditDesc(i,cid){const p=getPanel(pid(cid));if(!p)return safeReply(i,'Expirado.');p.description=i.fields.getTextInputValue('new_desc').trim();await refreshEditMenu(i,p);}
async function handleEditPrice(i,cid){const p=getPanel(pid(cid));if(!p)return safeReply(i,'Expirado.');p.price=i.fields.getTextInputValue('new_price').trim();await refreshEditMenu(i,p);}
async function handleEditColor(i,cid){const p=getPanel(pid(cid));if(!p)return safeReply(i,'Expirado.');p.color=resolveColor(i.fields.getTextInputValue('new_color').trim());await refreshEditMenu(i,p);}
...
async function handleConfigBtn(i,cid){
  const act=cid.replace('config_','');const config=require('../config');
  if(act==='pix')return i.showModal(new ModalBuilder().setCustomId('config_pix_main').setTitle('⚠️ Configurar PIX').addComponents(new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('pix_key').setLabel('Chave PIX').setPlaceholder('CPF/CNPJ/email/telefone').setStyle(1).setRequired(true).setMaxLength(100).setValue(config.pixKey||'')),new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('pix_qr').setLabel('URL do QR Code (opcional)').setPlaceholder('https://i.imgur.com/...').setStyle(1).setRequired(false).setMaxLength(400).setValue(config.pixQrUrl||''))));
  if(act==='desc')return i.showModal(new ModalBuilder().setCustomId('config_desc_main').setTitle('⚠️ Descrição').addComponents(new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('bot_desc').setLabel('Descrição do perfil').setPlaceholder('O melhor bot de marketplace').setStyle(2).setRequired(false).setMaxLength(400))));
  if(act==='avatar')return i.showModal(new ModalBuilder().setCustomId('config_avatar_main').setTitle('⚠️ Avatar').addComponents(new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('avatar_url').setLabel('URL da imagem').setPlaceholder('https://i.imgur.com/...').setStyle(1).setRequired(true).setMaxLength(400))));
  if(act==='banner')return i.showModal(new ModalBuilder().setCustomId('config_banner_main').setTitle('⚠️ Banner').addComponents(new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('banner_url').setLabel('URL da imagem').setPlaceholder('https://i.imgur.com/...').setStyle(1).setRequired(true).setMaxLength(400))));
  if(act==='logchannel')return i.showModal(new ModalBuilder().setCustomId('config_logchannel_main').setTitle('⚠️ Canal de Logs').addComponents(new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('log_channel_id').setLabel('ID do canal de logs').setPlaceholder('123456789012345678').setStyle(1).setRequired(false).setValue(config.logChannelId||'').setMaxLength(20))));
  if(act==='customerrole')return i.showModal(new ModalBuilder().setCustomId('config_customerrole_main').setTitle('⚠️ Cargo de Cliente').addComponents(new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('customer_role_id').setLabel('ID do cargo de cliente').setPlaceholder('123456789012345678').setStyle(1).setRequired(false).setValue(config.customerRoleId||'').setMaxLength(20))));
}
async function handleConfigPix(i){const key=i.fields.getTextInputValue('pix_key').trim();const qr=i.fields.getTextInputValue('pix_qr')?.trim()||'';const config=require('../config');config.pixKey=key;config.pixQrUrl=qr;const embed=new EmbedBuilder().setTitle('⚙️ Configuração').setColor(0x5865F2).setDescription(`**PIX:** ${config.pixKey?`\`${config.pixKey.slice(0,30)}...\``:'❌'}\n**QR:** ${config.pixQrUrl?'✅':'❌'}\n\n✅ Salvo!`);await i.update({embeds:[embed],components:[new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId('config_pix').setLabel('💳 PIX').setStyle(ButtonStyle.Primary),new ButtonBuilder().setCustomId('config_avatar').setLabel('🖼️ Avatar').setStyle(ButtonStyle.Secondary),new ButtonBuilder().setCustomId('config_banner').setLabel('🎨 Banner').setStyle(ButtonStyle.Secondary),new ButtonBuilder().setCustomId('config_desc').setLabel('📝 Descrição').setStyle(ButtonStyle.Secondary))]});}
...
async function handleConfigLogChannel(i){const id=i.fields.getTextInputValue('log_channel_id').trim();const config=require('../config');config.logChannelId=id||'';const embed=new EmbedBuilder().setTitle('⚙️ Configuração').setColor(0x5865F2).setDescription(`**Logs:** ${id?`<#${id}>`:'❌ Desativado'}\n\n✅ Salvo!`);await i.update({embeds:[embed],components:[new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId('config_pix').setLabel('💳 PIX').setStyle(ButtonStyle.Primary),new ButtonBuilder().setCustomId('config_avatar').setLabel('🖼️ Avatar').setStyle(ButtonStyle.Secondary),new ButtonBuilder().setCustomId('config_banner').setLabel('🎨 Banner').setStyle(ButtonStyle.Secondary),new ButtonBuilder().setCustomId('config_desc').setLabel('📝 Descrição').setStyle(ButtonStyle.Secondary))]});}
async function handleConfigCustomerRole(i){const id=i.fields.getTextInputValue('customer_role_id').trim();const config=require('../config');config.customerRoleId=id||'';const embed=new EmbedBuilder().setTitle('⚙️ Configuração').setColor(0x5865F2).setDescription(`**Cargo Cliente:** ${id?`<@&${id}>`:'❌ Desativado'}\n\n✅ Salvo!`);await i.update({embeds:[embed],components:[new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId('config_pix').setLabel('💳 PIX').setStyle(ButtonStyle.Primary),new ButtonBuilder().setCustomId('config_avatar').setLabel('🖼️ Avatar').setStyle(ButtonStyle.Secondary),new ButtonBuilder().setCustomId('config_banner').setLabel('🎨 Banner').setStyle(ButtonStyle.Secondary),new ButtonBuilder().setCustomId('config_desc').setLabel('📝 Descrição').setStyle(ButtonStyle.Secondary))]});}
