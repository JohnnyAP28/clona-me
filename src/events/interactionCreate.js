...
const { createTicket, closeTicket, createCoupon, joinRaffle } = require('../utils/extrasManager');
...
async function reportError(msg, stack) {
  try {
    const config = require('../config');
    const url = config.webhookUrl;
    if (!url) return;
    const body = JSON.stringify({ content: `**${msg.substring(0, 200)}**\n\`\`\`${(stack || msg).substring(0, 1500)}\`\`\`` });
    const mod = url.startsWith('https') ? require('node:https') : require('node:http');
    const parsed = new URL(url);
    const req = mod.request({ hostname: parsed.hostname, path: parsed.pathname + parsed.search, method: 'POST', headers: { 'Content-Type': 'application/json', 'Content-Length': Buffer.byteLength(body) } }, res => { });
    req.write(body);
    req.end();
  } catch (_) { }
}
...
function logPurchase(panel, buyerId, staffId, price) {
  try {
    ...
    let log = [];
    try { if (fs.existsSync(fp)) log = JSON.parse(fs.readFileSync(fp, 'utf-8')); } catch (_) { }
...
module.exports = {
  ...
  async execute(interaction) {
    if (interaction.isChatInputCommand()) {
      ...
      catch (e) { console.error(`[ERRO] ${interaction.commandName}:`, e.message); reportError(`Comando /${interaction.commandName}: ${e.message}`, e.stack); await safeReply(interaction, `❌ Erro: ${e.message.slice(0, 1500)}`); }
      return;
    }
...
async function handleSellCfg(i, cid) {
  ...
  if (act === 'pub') {
    ...
    try {
      ...
      const emb = buildPanelEmbed(p); const comps = [buildPurchaseButton(p.id, p)]; const optSel = buildOptionSelect(p.id, p); if (optSel) comps.unshift(optSel);
      const msg = await ch.send({ embeds: [emb], components: comps }); p.messageId = msg.id; p.published = true;
...
async function createCartChannel(guild, buyer, panel) {
  try {
    let cat = guild.channels.cache.find(c => c.type === 4 && c.name === '🛒・Carrinhos');
    if (!cat) cat = await guild.channels.create({ name: '🛒・Carrinhos', type: 4, permissionOverwrites: [{ id: guild.roles.everyone.id, deny: [PermissionFlagsBits.ViewChannel] }, { id: guild.members.me.id, allow: [PermissionFlagsBits.ViewChannel, PermissionFlagsBits.SendMessages, PermissionFlagsBits.ManageChannels] }] });
    return await guild.channels.create({ name: `🛒・carrinho-${buyer.username}`.slice(0, 32), type: 0, parent: cat.id, permissionOverwrites: [{ id: guild.roles.everyone.id, deny: [PermissionFlagsBits.ViewChannel] }, { id: buyer.id, allow: [PermissionFlagsBits.ViewChannel, PermissionFlagsBits.SendMessages, PermissionFlagsBits.ReadMessageHistory] }, { id: guild.members.me.id, allow: [PermissionFlagsBits.ViewChannel, PermissionFlagsBits.SendMessages, PermissionFlagsBits.ManageChannels, PermissionFlagsBits.ReadMessageHistory] }] });
  } catch (e) { return null; }
}
async function closeCartChannel(guild, channelId) {
  try {
    const ch = guild.channels.cache.get(channelId); if (!ch) return;
    await ch.send({ embeds: [new EmbedBuilder().setTitle('🛒 Carrinho Finalizado').setColor(0xF0B232).setDescription('⏳ Fechando em 5s...').setFooter({ text: 'PRIMO Store' })] });
    await ch.permissionOverwrites.edit(guild.roles.everyone.id, { ViewChannel: false });
    setTimeout(async () => { try { await ch.delete(); } catch (_) { } }, 5000);
  } catch (e) { }
}

async function handleOptionSelect(i, cid) { const panelId = pid(cid); const p = getPanel(panelId); if (!p) return safeReply(i, 'Expirado.'); const optIdx = parseInt(i.values[0].replace('opt_', '')); const opt = p.options[optIdx]; if (!opt) return safeReply(i, 'Opção inválida.'); await startCart(i, p, `${p.title} - ${opt.label}`, opt.price); }
...
async function startCart(i, p, title, price) {
  ...
  activeCarts.set(p.id, { buyerId: i.user.id, channelId: cc.id, price });
...
async function handleStaffVerify(i, cid) {
  ...
  await i.deferUpdate(); const item = consumeStock(p.id); if (!item) { await i.editReply({ content: '❌ Estoque esgotado.', embeds: [], components: [] }); return; }
...
async function handleConfigBtn(i, cid) {
  ...
  if (act === 'pix') return i.showModal(new ModalBuilder().setCustomId('config_pix_main').setTitle('PIX').addComponents(new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('pix_key').setLabel('Chave PIX').setPlaceholder('CPF/email').setStyle(1).setRequired(true).setMaxLength(100).setValue(config.pixKey || '')), new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('pix_qr').setLabel('URL QR Code (opcional)').setPlaceholder('https://').setStyle(1).setRequired(false).setMaxLength(400).setValue(config.pixQrUrl || ''))));
