  ...
  async execute(interaction) {
    if (interaction.isChatInputCommand()) {
      ...
      try { await cmd.execute(interaction); }
        ...
        console.error(`[ERRO] ${interaction.commandName}:`, e.message);
...
async function handlePayConfirm(i, cid) {
  ...
  if (p.deliveryType==='auto') {
    ...
    if (!item) return i.followUp({content:'❌ Estoque esgotado.',ephemeral:true});
    ...
    const emb=new EmbedBuilder().setTitle('🎁 Pagamento Confirmado').setColor(0x57f287).setDescription(`**${p.title}**\n\nAqui está seu produto:`).addFields({name:'📦 Conteúdo',value:item.slice(0,1024)}).setFooter({text:`Venda #${p.soldCount}`}).setTimestamp();
    return i.followUp({embeds:[emb],ephemeral:true});
  }
  const notif=new EmbedBuilder().setTitle('🔔 Aguardando Entrega').setColor(0x5865F2).setDescription(`**${p.title}**\n**Valor:** ${p.price}\n**Comprador:** ${i.user} (${i.user.id})\n\n⏳ Staff precisa confirmar.`).setFooter({text:`Painel #${p.id}`}).setTimestamp();
  if (p.channelId) { try { const ch=i.client.channels.cache.get(p.channelId); if(ch) { const r=new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId(`staffdeliver_${p.id}`).setLabel('📦 Confirmar Entrega').setStyle(ButtonStyle.Success)); await ch.send({content:`⚠️ Novo pagamento! ${i.user} comprou **${p.title}**.`,embeds:[notif],components:[r]}); } } catch(_){} }
  p.soldCount++;
  return i.followUp({content:'✅ Pagamento registrado! Aguarde a entrega.',ephemeral:true});
}
...
async function handleStaffDeliver(i, cid) {
  ...
  const p=getPanel(pid(cid)); if(!p) return safeReply(i, 'Painel expirado.');
  const item=consumeStock(p.id); if(!item) return safeReply(i, '❌ Estoque esgotado.');
  if(p.published&&p.messageId){try{const ch=i.client.channels.cache.get(p.channelId);const msg=await ch?.messages.fetch(p.messageId).catch(()=>null);if(msg)await msg.edit({embeds:[buildPanelEmbed(p)],components:[buildPurchaseButton(p.id,p)]});}catch(_){}}
...
async function handleEditConfigBtn(i, cid) {
  const pid2=pid(cid), p=getPanel(pid2); if(!p) return safeReply(i,'Painel expirado.');
...
async function handleEditUpdate(i, cid) {
  const pid2=pid(cid), p=getPanel(pid2); if(!p||!p.published) return safeReply(i, 'Painel não publicado.');
  await i.deferReply({ephemeral:true});
  try { const ch=i.client.channels.cache.get(p.channelId); if(!ch) return i.followUp({content:'Canal não encontrado.',ephemeral:true});
    ...
    if(!msg) return i.followUp({content:'Mensagem original perdida. Republique.',ephemeral:true});
...
async function handleConfigBtn(i, cid) {
  ...
  if (act === 'pix') return i.showModal(new ModalBuilder().setCustomId('config_pix_main').setTitle('Configurar PIX').addComponents(
    new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('pix_key').setLabel('Chave PIX').setPlaceholder('CPF/CNPJ/email/telefone').setStyle(1).setRequired(true).setMaxLength(100)),
  ...
  if (act === 'desc') return i.showModal(new ModalBuilder().setCustomId('config_desc_main').setTitle('Descrição do Bot').addComponents(
    new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('bot_desc').setLabel('Descrição do perfil').setPlaceholder('O melhor bot de marketplace').setStyle(2).setRequired(false).setMaxLength(400)),
  ));
  if (act === 'avatar') return i.showModal(new ModalBuilder().setCustomId('config_avatar_main').setTitle('Avatar do Bot').addComponents(
    new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('avatar_url').setLabel('URL da imagem do avatar').setPlaceholder('https://i.imgur.com/...').setStyle(1).setRequired(true).setMaxLength(400)),
  ));
  if (act === 'banner') return i.showModal(new ModalBuilder().setCustomId('config_banner_main').setTitle('Banner do Bot').addComponents(
