...
async function refreshEditMenu(i, panel) {
  ...
  const r4 = new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId(`editcfg_update_${panel.id}`).setLabel('🔄 Atualizar Publicação').setStyle(ButtonStyle.Success));
...
async function handleSellConfigBtn(i, cid) {
  ...
  if(act==='stock') return i.showModal(sell.buildStockModal(pid2)); if(act==='deliv') return i.showModal(sell.buildDeliveryModal(pid2,p.deliveryType)); if(act==='icon') return i.showModal(sell.buildIconModal(pid2,p.iconUrl)); if(act==='banner') return i.showModal(sell.buildBannerModal(pid2,p.bannerUrl)); if(act==='display') return i.showModal(sell.buildDisplayModal(pid2,p));
  ...
  if(act==='pub'){if(!p.title||!p.price) return safeReply(i,'Preencha título e valor.'); await i.deferReply({ephemeral:true}); try{const ch=i.client.channels.cache.get(p.channelId); if(!ch) return i.followUp({content:'❌ Canal não encontrado.',ephemeral:true}); const emb=buildPanelEmbed(p); const btn=buildPurchaseButton(p.id,p); const msg=await ch.send({embeds:[emb],components:[btn]}); p.messageId=msg.id; p.published=true; await i.followUp({content:`✅ Painel #${p.id} publicado em ${ch}.`,ephemeral:true});}catch(e){await i.followUp({content:`❌ Erro: ${e.message}`,ephemeral:true});}}
}
...
async function createCartChannel(guild, buyer, panel) {
  try {
    ...
    if (!cartCategory) {
      cartCategory = await guild.channels.create({name:'🛒・Carrinhos',type:4,permissionOverwrites:[{id:guild.roles.everyone.id,deny:[PermissionFlagsBits.ViewChannel]},{id:guild.members.me.id,allow:[PermissionFlagsBits.ViewChannel,PermissionFlagsBits.SendMessages,PermissionFlagsBits.ManageChannels]}]});
    }
    const channelName = `🛒・carrinho-${buyer.username}`.slice(0,32);
    const channel = await guild.channels.create({name:channelName,type:0,parent:cartCategory.id,
      permissionOverwrites:[{id:guild.roles.everyone.id,deny:[PermissionFlagsBits.ViewChannel]},{id:buyer.id,allow:[PermissionFlagsBits.ViewChannel,PermissionFlagsBits.SendMessages,PermissionFlagsBits.ReadMessageHistory]},{id:guild.members.me.id,allow:[PermissionFlagsBits.ViewChannel,PermissionFlagsBits.SendMessages,PermissionFlagsBits.ManageChannels,PermissionFlagsBits.ReadMessageHistory]}]});
    return channel;
...
async function closeCartChannel(guild, channelId) {
  try {
    ...
    // Aviso de fechamento
    await channel.send({ embeds: [new EmbedBuilder().setTitle('🛒 Carrinho Finalizado').setColor(0xF0B232).setDescription('⏳ **Este carrinho será fechado em 5 segundos...**').setFooter({text:'Obrigado por comprar na PRIMO Store!'})] });

    // Remove acesso do @everyone e bloqueia o canal
    await channel.permissionOverwrites.edit(guild.roles.everyone.id, { ViewChannel: false });

    // Deleta após 5 segundos
    setTimeout(async () => {
      try { await channel.delete('Compra finalizada'); } catch (_) {}
    }, 5000);
  } catch (e) { console.error('[CART] Erro ao fechar:', e.message); }
}
...
async function handleStaffVerify(i, cid) {
  ...
  await i.deferUpdate();
  const item=consumeStock(p.id); if(!item){await i.editReply({content:'❌ Estoque esgotado.',embeds:[],components:[]});return;}
  ...
  const dmLink='https://discord.com/channels/@me'; let dmSent=false;
  ...
  await i.editReply({content:null,embeds:[new EmbedBuilder().setTitle('🎉 Parabéns pela compra! 🎉').setColor(0x57f287).setDescription(`<@${buyerId}>, seu produto foi entregue com sucesso!\n\n📬 **Acesse seu DM:** [Clique aqui para abrir](${dmLink})\n🛒 Carrinho fechando em 5 segundos...`).setFooter({text:`Painel #${p.id} • Confirmado por ${i.user.username}`}).setTimestamp()],components:[]});
}
async function handleStaffReject(i, cid) {
  if(!i.memberPermissions?.has('Administrator')) return safeReply(i,'Apenas staff.');
  const parts=cid.split('_'); const panelId=parseInt(parts[1]); const buyerId=parts[2];
  const cart=activeCarts.get(panelId);
  if(cart){await closeCartChannel(i.guild,cart.channelId);activeCarts.delete(panelId);}
  await i.update({content:`❌ Pagamento do painel #${panelId} de <@${buyerId}> rejeitado. Carrinho fechando em 5s...`,embeds:[],components:[]});
}
async function handlePayCancel(i, cid) {
  const panelId=pid(cid); const cart=activeCarts.get(panelId);
  if(cart){await closeCartChannel(i.guild,cart.channelId);activeCarts.delete(panelId);}
  await i.update({content:'❌ Compra cancelada. Carrinho fechando em 5s...',embeds:[],components:[]});
}
...
async function handleConfigBtn(i, cid) {
  ...
  const pixStatus = config.pixKey ? `\`${config.pixKey.slice(0,30)}...\`` : '❌ Não configurado';
  const qrStatus = config.pixQrUrl ? '✅ Configurado' : '❌ Não configurado';
