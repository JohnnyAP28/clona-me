...
module.exports = {
  ...
  async execute(interaction) {
    ...
    // Lista painéis via select menu
    const options = panels.slice(0, 25).map(p => {
      const status = p.published ? '✅' : '📝';
      const label = `${status} #${p.id} — ${p.title}`.slice(0, 100);
      return new StringSelectMenuOptionBuilder()
        .setLabel(label)
        .setValue(`panel_${p.id}`)
        .setDescription(`Preço: ${p.price || 'N/D'} • ${p.stock.length} itens`);
    });

    const row = new ActionRowBuilder().addComponents(
      new StringSelectMenuBuilder()
        .setCustomId('edit_select_panel')
        .setPlaceholder('Selecione um painel para editar...')
        .addOptions(options)
    );

    // Também mostra opção de deletar múltiplos
    const deleteOptions = panels.slice(0, 25).map(p => {
      const label = `🗑️ Deletar: #${p.id} — ${p.title}`.slice(0, 100);
      return new StringSelectMenuOptionBuilder()
        .setLabel(label)
        .setValue(`delete_${p.id}`)
        .setDescription(`Publicado: ${p.published ? 'Sim' : 'Não'} • ${p.stock.length} itens`);
    });

    const deleteRow = new ActionRowBuilder().addComponents(
      new StringSelectMenuBuilder()
        .setCustomId('edit_delete_multi')
        .setPlaceholder('🗑️ Ou selecione painéis para DELETAR (múltipla escolha)...')
        .setMinValues(1)
    ...
    const embed = new EmbedBuilder()
      ...
      .setColor(0x5865F2)
      .setDescription(
        `**${panels.length} painel(is)** encontrado(s) neste servidor.\n\n` +
        `**Menu 1:** Selecione um painel para editar propriedades.\n` +
        `**Menu 2:** Selecione um ou mais painéis para deletar definitivamente.`
      )
...
async function showEditMenu(interaction, panel) {
  ...
  const r2 = new ActionRowBuilder().addComponents(
    ...
    new ButtonBuilder().setCustomId(`editcfg_items_${panel.id}`).setLabel('🔄 Substituir Itens').setStyle(ButtonStyle.Secondary),
  ...
  await interaction.reply({ embeds: [embed], components: [r1, r2, r3], ephemeral: true });
}
